...
class DisciplinaryRecordTest(unittest.TestCase):
    ...
    def test_disciplinary_record_creation(self.):
        #Login
        print("login")
        usrname = "adminjustin"
        driver.get(url)
        ...
        id = "1004000"
        record_type = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(random.randint(8, 15)))
        ...
        time.sleep(3)
        #check the record
        self.assertEqual(description,(get_text(driver, id+description)))
        ...
        self.assertEqual(points,(get_text(driver,id+description+"_points_deduction")))
        ...
        clear_input_text(driver,"edit_disciplinary_record_points_deduction")
        input_text(driver, "edit_disciplinary_record_description",description)
        input_text(driver,"edit_disciplinary_record_points_deduction",points)
        click_btn(driver,"edit_disciplinary_record_btn")
